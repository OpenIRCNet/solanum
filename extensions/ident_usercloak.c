...
static const char ident_usercloak_desc[] =
	"Insert the ident into certain iline spoofed hosts";

static void check_new_user(void *data);
mapi_hfn_list_av1 ident_usercloak_hfnlist[] = {
	{ "new_local_user", check_new_user },
	{ NULL, NULL }
};


unsigned int fnv_hash_string(const char *str)
{
...
check_new_user(void *vdata)
{
	...
	if (EmptyString(source_p->username))
		return;

	char *identpart = strstr(source_p->orighost, "/ident");
	if (!identpart || identpart[8] != '\0')
		return;
	...
	strncpy(buf, source_p->orighost, identpart - source_p->orighost);
	dst += identpart - source_p->orighost;

	int needhash = 0;

	for (const char *src = ident; *src ; src++ )
	{
		if (dst >= buf + sizeof(buf))
		{
			/* Doesn't fit. Warn opers and bail. */
			sendto_realops_snomask(SNO_GENERAL, L_NETWIDE,
					"Couldn't fit ident part %s in hostname for %s!%s@%s",
					ident, source_p->name, source_p->username, source_p->orighost);
			return;
		}
	...
	if (needhash)
	{
		...
		*dst++ = '-';
		...
		unsigned int hashval = fnv_hash_string(ident);
...
DECLARE_MODULE_AV2(ident_usercloak, NULL, NULL, NULL, NULL, ident_usercloak_hfnlist, NULL, NULL, ident_usercloak_desc);
